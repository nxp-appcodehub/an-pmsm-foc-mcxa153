...
void LPADC_SetConvCommandConfig(ADC_Type *base, uint32_t commandId, const lpadc_conv_command_config_t *config)
{
    ...
    /* If the device does not support differential operations, sampleChannelMode should not set as differential mode. */
    assert(((config->sampleChannelMode >= kLPADC_SampleChannelDiffBothSideAB) &&
            (((base->VERID) & ADC_VERID_DIFFEN_MASK) != 0UL)) ||
           (config->sampleChannelMode < kLPADC_SampleChannelDiffBothSideAB));
    /* If the device does not support two simultaneous single end conversions, sampleChannelMode should not as larger
        than #kLPADC_SampleChannelDiffBothSideBA. */
    assert(((config->sampleChannelMode > kLPADC_SampleChannelDiffBothSideBA) &&
            (((base->VERID) & ADC_VERID_NUM_SEC_MASK) != 0UL)) ||
           (config->sampleChannelMode <= kLPADC_SampleChannelDiffBothSideBA));
