...
------------------------------------------------------------------------------*/
#define CLK_GATE_REG_OFFSET(value) ((value) >> 16U)
#define CLK_GATE_BIT_SHIFT(value)  ((value)&0xFFFF)
...
#define CLK_ATTACH_REG_OFFSET(value) ((value) >> 16U)
#define CLK_ATTACH_CLK_SEL(value)    ((value)&0xFFFF)

#define CLK_ATTACH_MUX(reg, sel) ((reg << 16U) | (sel))
...
typedef enum _clock_attach_id
{
    ...
    kFRO16K_to_OSTIMER = CLK_ATTACH_MUX(kCLOCK_SelOSTIMER0, 0U), /*!< Attach FRO16K to OSTIMER0. */
...
static inline void CLOCK_EnableClock(clock_ip_name_t clk)
{
    ...
    volatile uint32_t *pClkCtrl = (volatile uint32_t *)((uint32_t)(&(MRCC0->MRCC_GLB_CCSET0)) + reg_offset);
    ...
    {
        SYSCON->PWM0SUBCTL |= (1 << bit_shift);
        CLOCK_EnableClock(kCLOCK_GateFLEXPWM0);
    }
    else
    {
        *pClkCtrl = (1 << bit_shift);
    }
...
static inline void CLOCK_DisableClock(clock_ip_name_t clk)
{
    uint32_t reg_offset         = CLK_GATE_REG_OFFSET(clk);
    uint32_t bit_shift          = CLK_GATE_BIT_SHIFT(clk);
    volatile uint32_t *pClkCtrl = (volatile uint32_t *)((uint32_t)(&(MRCC0->MRCC_GLB_CCCLR0)) + reg_offset);
    ...
    if (reg_offset == REG_PWM0SUBCTL)
    {
        ...
        if (0 == (SYSCON->PWM0SUBCTL & 0xFU))
        {
            CLOCK_DisableClock(kCLOCK_GateFLEXPWM0);
        }
    }
    else
    {
 ...
 */
uint32_t CLOCK_GetLptmrClkFreq();
